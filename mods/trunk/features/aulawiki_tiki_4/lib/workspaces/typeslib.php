...
//		$this->TikiLib($db);
	...
	function add_workspace_type($code, $name, $desc, $menuid, $active, $resources, $userwstype, $hide) {
		...
		$uid = md5(uniqid(rand()));
		$result = $this->query($query, array ($code, $name, $desc, $menuid, $active, $resources, $userwstype, $hide, $uid));
...
*/
		...
		$result = $this->query($query, array ($name, $desc, $menuid, $active, $userwstype, $hide,$id));
		return true;
	}
	...
	function update_workspace_type_resources($id, $resources) {
		...
		$result = $this->query($query, array ($resources, $id));
		return true;
	}
	...
	function get_workspace_type_roles($typeId) {
		...
		$result = $this->query($query, array ($typeId));
		$ret = array ();
	...
	function add_workspace_type_roles($typeId, $roles, $code) {
		...
		foreach ($roles as $key => $rol) {
			...
			$result = $this->query($query, array ($rol, $typeId));
			/* 
				global $userlib;
	...
	function add_workspace_type_role($typeId, $roleName, $permGroup) {
		...
		$result = $this->query($query, array ($roleName, $typeId,$permGroup));
			/* 
				global $userlib;
				$userlib->add_group("RWSTYPEGRP".$code."-".$rol, $code."-".$rol." role&workspace type members", '');
				$eduuserlib->remove_inclusion("RWSTYPEGRP".$code."-".$rol, "ROLEGRP".$rol);
