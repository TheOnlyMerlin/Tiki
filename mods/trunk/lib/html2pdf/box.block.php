<?php
// $Header: /cvsroot/tikiwiki/_mods/lib/html2pdf/box.block.php,v 1.1 2008-01-15 09:20:24 mose Exp $
...
class BlockBox extends GenericContainerBox
{
   ...
   * definition to the GenericBox class, leaving only box-specific 'reflow_static' definitions in specific classes.
   *
  ...
  function reflow_absolute(&$context) {
    GenericBox::reflow($this->parent, $context);
    ...
    if (is_null($this->left) && is_null($this->right)) {
      ...
      $this->put_left($this->left + $containing_block['left'] + $this->get_extra_left());
    };
    ...
    if (is_null($this->top) && is_null($this->bottom)) {
      ...
      $this->put_top($containing_block['top'] - $this->top[0] - $this->get_extra_top());
    };
  ...
  function reflow_static_normal(&$parent, &$context) {
    ...
    if ($parent) { 
       ...
       * Collapse top margin
       *
       * @see GenericBox::collapse_margin()
       *
  ...
  function show_fixed(&$driver) {
     ...
     */
    $this->moveto($driver->left + $this->get_css_left_value(),
                  $driver->bottom + $driver->height + $driver->offset - $this->top[0]);
