...
if (!class_exists('FPDF')) {
  ...
  class PDFIndirectObject {
    ...
    function PDFIndirectObject($object_id, $generation_id) {
  ...
  class PDFAppearanceStream extends PDFIndirectObject {
    ...
    function pdf(&$handler) {
      ...
      $content .= "\n";
      $content .= $handler->_stream($this->content);
  ...
  class PDFAnnotationExternalLink extends PDFAnnotation {
    ...
    function &create(&$handler, $x, $y, $width, $height, $link, $generation_id = 0) {
      $annotation = new PDFAnnotationExternalLink($handler->_generate_new_object_number(),
                                                  $x, $y, $width, $height, $link,
                                                  $generation_id);
      return $annotation;
    }

    function PDFAnnotationExternalLink($object_id, 
                                       $x,
                                       $y,
                                       $width,
                                       $height,
                                       $link,
                                       $generation_id) {
      $this->PDFAnnotation($object_id, $generation_id);

      $this->rect = new PDFRect($x,$y,$width,$height);
      $this->link = $link;
    }
  ...
  class PDFAnnotationInternalLink extends PDFAnnotation {
    ...
    function &create(&$handler, $x, $y, $width, $height, $link, $generation_id = 0) {
      $annotation = new PDFAnnotationInternalLink($handler->_generate_new_object_number(),
                                                  $x, $y, $width, $height, $link,
                                                  $generation_id);
      return $annotation;
    }                

    function PDFAnnotationInternalLink($object_id, 
                                       $x,
                                       $y,
                                       $width,
                                       $height,
    ...
    function pdf(&$handler) {
      ...
      $content = $handler->_dictionary(array(
                                             ...
                                             'Rect'    => $this->rect->pdf($handler),
                                             'Border'  => "[0 0 0]",
                                             'Dest'    => sprintf("%d 0 R /XYZ 0 %.2f null",
                                                                  1+2*$l[0],
                                                                  $h-$l[1]*$handler->k)
                                             ));
      return $content;
  ...
  class PDFAnnotationWidget extends PDFAnnotation {
    ...
    function PDFAnnotationWidget($object_id, 
                                 $rect, 
                                 $generation_id) {
      $this->PDFAnnotation($object_id, $generation_id);
    ...
    function _out_nested(&$handler) {
      if (!is_null($this->appearance)) {
        $this->appearance->out($handler);
      };
    }
  }
  ...
  class PDFFieldCheckBox extends PDFAnnotationWidget {
    function PDFFieldCheckBox($object_id, 
                              $rect, 
                              $generation_id) {
      $this->PDFAnnotationWidget($object_id, 
                                 $rect, 
                                 $generation_id);
    }
    ...
    function &create(&$handler, 
                     $rect, 
                     $generation_id=0) {
      $field = new PDFFieldCheckBox($handler->_generate_new_object_number(), 
                                    $rect, 
                                    $generation_id);
      $field->appearance = new PDFAppearanceStream($handler->_generate_new_object_number(), 
    ...
    function pdf(&$handler) {
      $content = $handler->_dictionary(array(
                                             ...
                                             'Rect'    => 
                                             "[".
                                             $this->rect->left($handler)  ." ".
                                             $this->rect->top($handler)   ." ".
                                             $this->rect->right($handler) ." ".
                                             $this->rect->bottom($handler).
                                             "]",
                                             'FT'      => '/Btn',
                                             'Ff'      => sprintf("%d", 0),
                                             'T'       => sprintf("(FieldObject%d)",$this->get_object_id()),
                                             'TU'      => "<FEFF>",
                                             'MK'      => "<< /CA (8) >>"
                                             ));
      return $content;
  ...
  class PDFFieldPushButton extends PDFAnnotationWidget {
    ...
    function PDFFieldPushButton($object_id, 
                                $rect, 
                                $fontindex, 
                                $fontsize, 
                                $link,
    ...
    function &create(&$handler, 
                     ...
                     $link,
                     $generation_id=0) {
      $field = new PDFFieldPushButton($handler->_generate_new_object_number(), 
                                      $rect, 
                                      $fontindex,
                                      $fontsize,
                                      $link,
    ...
    function pdf(&$handler) {
//"<< /U << /S /URI /URI ".$handler->_textstring($this->link)." >> >>"
      ...
      if (is_null($this->link)) {
        ...
        $action = $handler->_dictionary(array(
                                              'S'     => "/SubmitForm",
                                              'F'     => $handler->_textstring($this->link),
                                              'Flags' => (1 << 1) | // IncludeNoValueFields
      ...
      $content = $handler->_dictionary(array(
                                             ...
                                             "]",
                                             'FT'      => '/Btn',
                                             //                   'H'       => '/P', // Display the annotation's down appearance
                                             'Ff'      => sprintf("%d", 1 << 16),
                                             'T'       => sprintf("(FieldObject%d)",$this->get_object_id()),
                                             'TU'      => "<FEFF>",
                                             'DR'      => "2 0 R",
                                             'DA'      => sprintf("(0 0 0 rg /F%d %.2f Tf)", 
                                                                  $this->fontindex,
                                                                  $this->fontsize),
                                             'AP'      => "<< /N ".$handler->_reference($this->appearance)." >>",
                                             'AA'      => $action
                                             ));
      return $content;
    }
  ...
  class PDFFieldRadio extends PDFAnnotationWidget {
    ...
    function &create(&$handler, 
                     $rect, 
                     $generation_id=0) {
      $field = new PDFFieldRadio($handler->_generate_new_object_number(), 
                                 $rect, 
                                 $generation_id);
      return $field;
    }
    ...
    function pdf(&$handler) {
      $content = $handler->_dictionary(array(
                                             ...
                                             "]",
                                             'MK'      => "<< /CA (8) >>",
                                             'Parent'  => $handler->_reference($this->parent),
                                             ));
      return $content;
    }
  ...
  class PDFFieldRadioGroup extends PDFIndirectObject {
    ...
    /**
     * @var Array Contains all radio buttons in this group
     * @access private
     */
    var $_radios;
    ...
    function &create(&$handler, 
                     $group_name,
                     $generation_id=0) {
      $field = new PDFFieldRadioGroup($handler->_generate_new_object_number(), 
                                      $group_name,
                                      $generation_id);
      return $field;
    }
    ...
    function _out_nested(&$handler) {
      $size = count($this->_radios);
      for ($i=0; $i<$size; $i++) {
        $this->_radios[$i]->out($handler);
      }
    ...
    function pdf(&$handler) {
      $content = $handler->_dictionary(array(
                                             ...
                                             'Ff'      => sprintf("%d", 1 << 15),
                                             'T'       => sprintf("(FieldObject%d)",$this->get_object_id()),
                                             'Kids'    => $handler->_reference_array($this->_radios)
                                             ));
      return $content;
    }

    function PDFFieldRadioGroup($object_id, 
                                $group_name,
                                $generation_id) {
      $this->PDFIndirectObject($object_id, $generation_id);
    }
  ...
  class PDFFieldText extends PDFAnnotationWidget {
    ...
    function PDFFieldText($object_id, 
                          $rect, 
                          $fontindex, 
                          $fontsize, 
                          $value,
    ...
    function &create(&$handler, 
                     ...
                     $value,
                     $generation_id=0) {
      $field = new PDFFieldText($handler->_generate_new_object_number(), 
                                $rect, 
                                $fontindex,
                                $fontsize,
                                $value,
  ...
  class FPDF {
    ...
    function add_field_checkbox($x, $y, $w, $h) {
      $form =& PDFFieldCheckBox::create($this, 
                                        new PDFRect($x, $y, $w, $h));

      $this->_form_fields[] =& $form;
      $this->_pages[count($this->_pages)-1]->add_annotation($form);    
    
      return count($this->_form_fields)-1;
    }

    function add_field_radio($x, $y, $w, $h, $group_name) {
      if (isset($this->_form_radios[$group_name])) {
        $radio_group =& $this->_form_radios[$group_name];
      } else {
        $radio_group =& PDFFieldRadioGroup::create($this, $group_name);
        $this->_form_fields[] =& $radio_group;
        $this->_form_radios[$group_name] =& $radio_group;
      };

      $radio =& PDFFieldRadio::create($this, 
                                      new PDFRect($x, $y, $w, $h));
    ...
    function add_field_text($x, $y, $w, $h, $value) {
      $form =& PDFFieldText::create($this, 
                                    new PDFRect($x, $y, $w, $h), 
                                    $this->CurrentFont['i'], 
                                    $this->FontSizePt,
                                    $value);
    ...
    function add_field_pushbutton($x, $y, $w, $h, $actionURL) {
      $form =& PDFFieldPushButton::create($this, 
                                          new PDFRect($x, $y, $w, $h), 
                                          $this->CurrentFont['i'], 
                                          $this->FontSizePt,
                                          $actionURL);
    ...
    function AddPage($orientation='') {
      $this->_pages[] =& new PDFPage(null, null);
    ...
    function add_link_external($x, $y, $w, $h, $link) {
      $this->_pages[count($this->_pages)-1]->add_annotation(PDFAnnotationExternalLink::create($this,$x,$y,$w,$h,$link));
    }
    ...
    function add_link_internal($x, $y, $w, $h, $link) {
      $this->_pages[count($this->_pages)-1]->add_annotation(PDFAnnotationInternalLink::create($this,$x,$y,$w,$h,$link));      
    }    
    ...
    function _putpages() {
      ...
      $kids='/Kids [';
      for ($i=0; $i<$nb; $i++) {
        $kids .= ($pages_start_obj_number+2*$i).' 0 R ';
      };
      $this->_out($kids.']');
    ...
    function _putcatalog($pages_start_obj_number) {
      ...
      if (count($this->_form_fields) > 0) {
        $this->_out('/AcroForm <<');
        $this->_out('/Fields '.$this->_reference_array($this->_form_fields));
        $this->_out('/DR 2 0 R');
    ...
    function _enddoc() {
      ...
      // Form fields
      for ($i=0; $i<count($this->_form_fields); $i++) {
        $form =& $this->_form_fields[$i];
