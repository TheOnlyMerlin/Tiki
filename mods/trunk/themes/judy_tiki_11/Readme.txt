{* $Id$ *}
-=Theme details=-
* __Name:__ Judy (judy.css)
* __Author and maintainer:__  Adapted for Tiki Wiki CMS Groupware by Gary Cunningham-Lee (chibaguy).
* __License:__ GNU/LPGL. This theme is a derived work using visual elements of the Garland theme for Drupal, which was released under the GPL license.
* __Author URL:__ http://zukakakina.com
* __Description__: Judy is an implementation of the Garland theme for Drupal, for Tiki Wiki CMS Groupware. For more information, please see the documentation pages for this theme. 

-=Files=-
* styles/judy/Readme.txt
* styles/judy.css
* styles/judy/bg-content.png
* styles/judy/bg-content-left.png
* styles/judy/bg-content-right.png
* styles/judy/bg-navigation-item.png
* styles/judy/bg-navigation-item-hover.png
* styles/judy/bg-topbar.jpg
* styles/judy/duplicon-watermark.png
* styles/judy/gradient-inner.png
* styles/judy/header.png
* styles/judy/ie7.css
* styles/judy/index.php
* styles/judy/judy.png
* styles/judy/middle.png
* styles/judy/pics/icons/folder.png
* styles/judy/pics/icons/index.php
* styles/judy/pics/icons/ofolder.png
* templates/styles/judy/credits.tpl
* templates/styles/judy/index.php
* templates/styles/judy/tiki.tpl
 
-=Documentation=-
http://themes.tiki.org/Judy
http://zukakakina.com/Judy
See http://themes.tiki.org/Upgrading for overview of changes in this Tiki version related to themes.
 
-=Version=-
* Tiki 11
 
-=Changelog=-
6 updated for Tiki 11
5 updated for Tiki 7
4 updated for Tiki 6
3 updated for Tiki 5
2 updated for Tiki 4
1 initial package