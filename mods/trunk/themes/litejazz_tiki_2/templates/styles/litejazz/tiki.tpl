	...
	{/if}
		<table width="100%" cellpadding="0" cellspacing="0" id="main">
			<tr id="cols">
				...
				{/if}
				{if $prefs.feature_siteidentity eq 'y' and $prefs.feature_topbar_custom_code}		<div class="clearfix" id="topbar_custom_code">{eval var=$prefs.feature_topbar_custom_code}</div>{/if}				
				</td>
			...
			{if $smarty.session.fullscreen neq 'y'}
      		{if $prefs.feature_left_column eq 'user' or $prefs.feature_right_column eq 'user'}
				...
				<div style="text-align:left;float:left;"><a class="flip" href="#" onclick="toggleCols('leftcolumn','table-cell'); return false">{icon _id=ofolder  name="leftcolumnicn" class="colflip" alt="+/-"}&nbsp;{tr}Show/Hide Left Menus{/tr}&nbsp;</a></div>
					{/if}
					{if $prefs.feature_right_column eq 'user'&& $right_modules|@count > 0 && $show_columns.right_modules ne 'n'}
        				<div style="text-align:right;float:right;"><a class="flip" href="#" onclick="toggleCols('rightcolumn','table-cell'); return false"> &nbsp;{tr}Show/Hide Right Menus{/tr}&nbsp;{icon _id=ofolder name="rightcolumnicn" class="colflip" alt="+/-"}</a>
						</div>
					{/if}
        			<br clear="all" />
					</div>
